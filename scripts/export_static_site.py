...
def repair_text_artifacts(content: str) -> str:
    if any(marker in content for marker in ("Ø", "Ù", "Ã", "â€", "â€”", "â€“")):
        try:
...
def render_page(*, template_name: str, output_path: Path, lang: str, route_name: str, current_page: str, context: dict, slug: str | None = None) -> None:
    ...
    html = html.replace("\ufeff", "")
    html = repair_text_artifacts(html)
...
def legal_content(lang: str) -> dict[str, dict[str, str]]:
    if lang == "ar":
        return {
            "terms": {
                "title": "شروط الاستخدام - MaharaLink",
                "kicker": "شروط الاستخدام",
                "heading": "شروط استخدام نسخة العرض الثابتة",
                "intro": "هذه نسخة عرض ثابتة مخصصة للنشر عبر GitHub Pages.",
                "content": "<p>المحتوى المعروض هنا يهدف إلى تقديم نظرة عامة على المنصة والدورات والخدمات بشكل تعريفي.</p><p>لبدء مشروعك أو طلب تجربة كاملة، يرجى التواصل عبر حساب مستقل الموضح داخل الموقع.</p>",
            },
            "privacy": {
                "title": "سياسة الخصوصية - MaharaLink",
                "kicker": "الخصوصية",
                "heading": "سياسة الخصوصية",
                "intro": "يتم عرض هذه النسخة الثابتة عبر GitHub Pages دون معالجة تسجيلات أو بيانات مستخدمين مباشرة.",
                "content": "<p>لا تجمع هذه النسخة الثابتة بيانات حسابات أو مدفوعات أو طلبات اشتراك من الزوار.</p><p>أي تواصل يتم خارج الموقع عبر القنوات الخارجية المعروضة مثل حساب مستقل.</p>",
            },
            "cookies": {
                "title": "سياسة ملفات الارتباط - MaharaLink",
                "kicker": "ملفات الارتباط",
                "heading": "سياسة ملفات الارتباط",
                "intro": "قد يعتمد عرض الصفحة على ملفات تقنية بسيطة خاصة بالمتصفح أو GitHub Pages.",
                "content": "<p>لا تستخدم هذه النسخة نظام تتبع تسويقي معقد داخل صفحات العرض الحالية.</p><p>قد تستخدم الخدمات الخارجية المضمّنة سياسات تقنية مستقلة وفق إعدادات المتصفح أو المنصة المستضيفة.</p>",
            },
        }
